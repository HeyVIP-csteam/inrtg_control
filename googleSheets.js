...
export async function onRequestPost({ request, env }) {
  ...
  if (Array.isArray(attachments) && attachments.length) {
    for (const att of attachments.slice(0, 10)) {
      try {
        const link = await sendTelegramAttachment({ botToken, route, attachment: att });
    ...
    try {
      ...
      if (layout) {
        ...
        const values = layout.columns.map((col) => {
          ...
          const [specialKey, fallbackKeys] = Object.entries(col)[0];
...
async function sendTelegramAttachment({ botToken, route, attachment }) {
  ...
  return buildMessageLink(route, data.result.message_id);
}
